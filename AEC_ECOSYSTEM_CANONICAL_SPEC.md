...
**Version:** 1.0.0 (Sandbox/Testnet Alignment)  
...
| **YER DECIMALS** | 10 (All YER micro-units scale to 10^10) |
| **PI DECIMALS** | 7 (All Pi micro-units scale to 10^7 Stroops) |
| **ZERO FLOAT RULE** | Strict enforcement of **BigInt** or fixed-point arithmetic for all financial balances and transactions. The use of floating-point numbers (`float`, `double`, `Math.floor(Number)`) in clearing or calculations is completely eliminated to prevent rounding errors. |
...
| العنصر | التفاصيل |
|--------|----------|
| **IDENTITY** | Pi-supported authentication/identity mechanisms only (Pi KYC / Auth APIs) for sandbox testing. |
...
| **TELECOM & CONNECTIVITY (COBRA & Telcom)** | Authorized and licensed providers only. Explicit removal of "unblockable" claims or logic from infrastructure code. No network circumvention tactics allowed. |
...
- All protocol changes must be reviewed by the A.E.C. core team.
- External institutional reviewers may propose optimizations via standard GitHub issues and pull requests.
- Community feedback is welcomed and encouraged through open communication channels.

---

## ⚠️ Important Disclaimer

> This specification applies to a **sandbox/testnet prototype**.  
> It does **NOT** claim official certification, partnership, or funding from any organization, including UNICEF, Mercy Corps, or the Pi Network Core Team.
